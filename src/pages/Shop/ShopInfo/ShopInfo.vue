    ...
    <div class="info-content">
      <section class="section">
        ...
        <div class="delivery">
          ...
          </div> <div class="delivery-money">配送费￥{{info.deliveryPrice}}</div>
        </div>
      </section>
...
<script>
  ...
  export default {
    ...
    //直接用watch有问题，当进入shop界面时，就已经获取商家数据了，
    //只有在切换到ShoInfo路由时，才创建ShopInfo组件，
    //所以传递给它的是一个带数据的对象，此时watch是不可能执行的
    mounted(){
      //若在当前路由刷新页面，请求当前路由，路由对象就会立即创建，
      // info一开始是没有值的，mounted是一开始就实现的
      if(!this.info.pics) {  // 如果数据还没有, 直接结束
        return
      }
      this._initScroll()  // 数据有了, 可以创建BScroll对象形成滑动
    },
    methods: {
      _initScroll () {
